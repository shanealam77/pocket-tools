...
export const Home: React.FC = () => {
  ...
  const filteredTools = useMemo(() => {
    const queryParts = search.toLowerCase().trim().split(/\s+/).filter(part => part.length > 0);
    
    if (queryParts.length === 0) return TOOLS;

    return TOOLS.filter(tool => {
      const searchContent = `${tool.name} ${tool.description} ${tool.category} ${tool.shortDescription}`.toLowerCase();
  ...
  return (
    <div className="min-h-screen pb-20">
      <SEO 
        title="Home" 
        description="Pocket Tools - A lightweight, mobile-first collection of essential online tools. QR generator, age calculator, password generator and more." 
      />
      ...
      <section className="relative px-6 pt-16 pb-12 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center relative z-10">
          ...
          <h1 className="text-4xl md:text-6xl font-black tracking-tight text-slate-900 dark:text-white mb-6">
            All your essential tools in <span className="text-indigo-600 dark:text-indigo-400">one pocket.</span>
          </h1>
          <p className="max-w-2xl mx-auto text-slate-500 dark:text-slate-400 text-lg md:text-xl mb-10 leading-relaxed">
            Fast, secure, and mobile-first. No tracking, just tools that work instantly.
          </p>
          ...
          <div className="max-w-xl mx-auto relative group">
            ...
            {!search && (
              <div className="absolute right-6 top-1/2 -translate-y-1/2 hidden md:flex items-center gap-2 pointer-events-none">
                <kbd className="px-2 py-1 bg-slate-100 dark:bg-slate-800 rounded border border-slate-200 dark:border-slate-700 text-[10px] font-bold text-slate-400">⌘</kbd>
                <kbd className="px-2 py-1 bg-slate-100 dark:bg-slate-800 rounded border border-slate-200 dark:border-slate-700 text-[10px] font-bold text-slate-400">K</kbd>
              </div>
            )}
          </div>
        </div>
        ...
        {favorites.length > 0 && !search && (
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xl font-bold tracking-tight">Favorite Tools</h2>
            </div>
      ...
      <section className="px-6 max-w-6xl mx-auto mt-8">
            ...
            <motion.div key={tool.id} variants={item}>
              ...
              >
                ...
                <div className="mt-auto pt-6 flex items-center justify-between">
                   <span className="text-[10px] font-black uppercase tracking-widest text-slate-300 dark:text-slate-600">{tool.category}</span>
        ...
        {filteredTools.length === 0 && (
          <div className="text-center py-20 px-6 bg-slate-100/50 dark:bg-slate-900/50 rounded-[40px] border-2 border-dashed border-slate-200 dark:border-slate-800">
            <h3 className="text-xl font-bold mb-2">No tools found for "{search}"</h3>
      ...
      <section className="px-6 max-w-4xl mx-auto mt-32 space-y-20">
        <div className="grid md:grid-cols-3 gap-12 text-center">
          <div className="space-y-4">
            ...
            <p className="text-sm text-slate-500 leading-relaxed">Compressed assets and smart routing ensure sub-second loading even on slow 3G.</p>
          </div>
          <div className="space-y-4">
            ...
            <p className="text-sm text-slate-500 leading-relaxed">All processing happens in your browser. We never see your data or inputs.</p>
          </div>
          <div className="space-y-4">
            ...
            <p className="text-sm text-slate-500 leading-relaxed">Designed for your pocket. Large touch targets and finger-friendly layouts.</p>
          </div>
        </div>
        ...
        <div className="p-10 bg-slate-900 dark:bg-black rounded-[40px] text-white overflow-hidden relative">
          ...
          <div className="relative z-10 max-w-2xl">
            <h2 className="text-3xl font-black mb-6">Why Pocket Tools?</h2>
            <p className="text-slate-400 mb-8 leading-relaxed">
              We believe the best tools should be invisible. Pocket Tools was built to eliminate the clutter of modern websites. No subscriptions, and definitely no login walls. Just the utility you need, when you need it.
            </p>
            <div className="flex flex-wrap gap-4">
               {['Free Forever', 'Open Source', 'Fast Support', 'Dark Mode'].map(tag => (
                 <span key={tag} className="px-4 py-2 bg-white/10 rounded-xl text-xs font-bold uppercase tracking-widest">
                   {tag}
                 </span>
               ))}
            </div>
      ...
      <footer className="mt-32 px-6 py-12 border-t border-slate-200 dark:border-slate-800 text-center">
         <p className="text-sm text-slate-400 font-medium">Made with ❤️ for the mobile web • Pocket Tools 2026</p>
      </footer>
    </div>
  );
};
