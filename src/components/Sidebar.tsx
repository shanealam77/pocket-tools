...
export const Sidebar: React.FC<{ className?: string }> = ({ className }) => {
  ...
  const filteredTools = useMemo(() => {
    ...
    return TOOLS.filter(tool => {
      const searchContent = `${tool.name} ${tool.shortDescription} ${tool.category}`.toLowerCase();
  ...
  return (
    <aside className={cn("flex flex-col h-screen border-r border-slate-200 bg-slate-50/50 backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/50 transition-colors", className)}>
      <div className="p-6 border-bottom border-slate-200 dark:border-slate-800">
        <Link to="/" className="flex items-center gap-2 mb-6">
          ...
          <span className="font-bold text-xl tracking-tight text-slate-900 dark:text-white">Pocket Tools</span>
        </Link>
      ...
      <nav className="flex-1 overflow-y-auto px-4 py-2 space-y-1">
        ...
        {favoritedTools.length > 0 && !search && (
          <>
            <div className="pt-4 pb-2 px-3 flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Favorites</span>
            </div>
        ...
        <div className="pt-4 pb-2 px-3">
          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">All Tools</span>
        </div>
        ...
        {filteredTools.map((tool) => (
          ...
          >
            <Icon name={tool.icon} className={cn("w-4 h-4 transition-transform group-hover:scale-110")} />
            {tool.name}
          </NavLink>
        ))}
      </nav>
      ...
      <div className="p-4 border-t border-slate-200 dark:border-slate-800">
        <p className="text-[10px] text-center text-slate-400">© 2026 Pocket Tools • Fast & Minimal</p>
      </div>
    </aside>
  );
};
